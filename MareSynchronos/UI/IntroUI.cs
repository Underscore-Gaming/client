...
public partial class IntroUi : WindowMediatorSubscriberBase
{
    ...
    public IntroUi(ILogger<IntroUi> logger, UiSharedService uiShared, MareConfigService configService,
        ...
        PerformanceCollectorService performanceCollectorService, DalamudUtilService dalamudUtilService) : base(logger, mareMediator, "Unsung Sync Setup", performanceCollectorService)
    {
        _uiShared = uiShared;
    ...
    protected override void DrawInternal()
    {
        ...
        {
            _uiShared.BigText("Welcome to Unsung Sync");
            ImGui.Separator();
            UiSharedService.TextWrapped("Unsung Sync is a plugin that will replicate your full current character state including all Penumbra mods to other paired Unsung Sync users. " +
                     ...
                     || !Directory.Exists(_configService.Current.CacheFolder)))
        {
            ...
            {
                UiSharedService.TextWrapped("To not unnecessary download files already present on your computer, Unsung Sync will have to scan your Penumbra mod directory. " +
                                     "Additionally, a local storage folder must be set where Unsung Sync will download other character files to. " +
                                     "Once the storage folder is set and the scan complete, this page will automatically forward to registration at a service.");
                UiSharedService.TextWrapped("Note: The initial scan, depending on the amount of mods you have, might take a while. Please wait until it is completed.");
                UiSharedService.ColorTextWrapped("Warning: once past this step you should not delete the FileCache.csv of Unsung Sync in the Plugin Configurations folder of Dalamud. " +
            ...
            if (!_dalamudUtilService.IsWine)
            {
                ...
                UiSharedService.ColorTextWrapped("The File Compactor can save a tremendeous amount of space on the hard disk for downloads through Unsung Sync. It will incur a minor CPU penalty on download but can speed up " +
                    "loading of other characters. It is recommended to keep it enabled. You can change this setting later anytime in the Unsung Sync settings.", ImGuiColors.DalamudYellow);
            }
        }
        else if (!_uiShared.ApiController.ServerAlive)
        {
            ...
            ImGui.Separator();
            UiSharedService.TextWrapped("To be able to use Unsung Sync you will have to register an account.");
            UiSharedService.TextWrapped("For the official Unsung Sync Servers the account creation will be handled on the official Unsung Sync Discord. Due to security risks for the server, there is no way to handle this sensibly otherwise.");
            UiSharedService.TextWrapped("If you want to register at the main server \"" + WebAPI.ApiController.MainServer + "\" join the Discord and follow the instructions as described in #how-to.");

            if (ImGui.Button("Join the Unsung Sync Discord"))
            {
                Util.OpenLink("https://discord.gg/");
            }
